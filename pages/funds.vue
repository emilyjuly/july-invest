...
onMounted(async () => {
    if (store.quoteListFund.length === 0) {
        await store.getQuoteList()
    }
    filteredData.value = [...store.quoteListFund]
...
const handleSearch = (value) => {
    filteredData.value = store.quoteListFund.filter(item => item.stock.toLowerCase().includes(value))
}
